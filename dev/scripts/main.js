...
let currentStation = 'ShJAbFEOMvE';
...
app.currentStation = () => {
    for (var i = 0; i < app.searchResults.length; i++) {
        if (app.searchResults[i].id.videoId === currentStation) {
            const index = app.searchResults.indexOf(app.searchResults[i])
            return index
        }
    }
}
...
app.loadStations = () => {
    const stationInfo = {}
    app.searchResults.forEach((station) => {
        app.stations.append(`
...
const stationChange = (shuffle, val) => {
    if (shuffle === false) {
        currentStation = app.searchResults[app.currentStation() + val].id.videoId
    } else{
        const randomStation = Math.floor(Math.random() * app.searchResults.length + 1)
...
app.overlay = () => {
    $('.gif').on('click', function(){
        const clickedUrl = $(this).find('img').data("gifUrl");
...
app.playPause = () => {
    ...
    app.$overlay.on('click', function(){
        const playerState = player.j.playerState
        if (playerState === 1) {
            player.pauseVideo()
